...
implementation //###############################################################
  ...
  procedure Show_List(var LH : real_LH);
    ...
    begin
      ...
      FE := LH;                     // Резервируем нынешный первый элемент, чтобы после к нему вернуться
  ...
  function GVoLE(var LH : real_LH): real;
    ...
    begin
      ...
      // Пока не наткнёмся на "пустоту":
      while LH <> Nil do
        begin
        
          if LH^.Next = Nil then      // Если после элемента ничего не идёт, то: 
          
            result := LH^.Content;    // это наша остановочка!
